...
func dataSourceOutscaleOAPIAccountConsumption() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceOutscaleOAPIAccountConsumptionRead,
			...
			"consumption_entries": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"consumption_value": &schema.Schema{
							Type:     schema.TypeInt,
							Computed: true,
						},
					},
...
func dataSourceOutscaleOAPIAccountConsumptionRead(d *schema.ResourceData, meta interface{}) error {
	...
	for k, v := range getResp.Entries {
		...
		entry["entry"] = aws.StringValue(v.Operation)
		entry["service"] = aws.StringValue(v.Service)
		entry["short_description"] = aws.StringValue(v.Title)
		entry["type"] = aws.StringValue(v.Type)
		entry["consumption_value"] = aws.Float64Value(v.Value)

		entries[k] = entry
	}

	d.SetId(resource.UniqueId())
	d.Set("consumption_entries", entries)
