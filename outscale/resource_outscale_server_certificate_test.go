...
func TestAccOutscaleEIMServerCertificate_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccEIMServerCertConfigFile(rInt, unixFile),
...
func testAccCheckCertExists(n string, cert *eim.ServerCertificate) resource.TestCheckFunc {
...
func testAccCheckEIMServerCertificateDestroy(s *terraform.State) error {
	conn := testAccProvider.Meta().(*OutscaleClient).EIM
...
func testAccEIMServerCertConfigFile(rInt int, fName string) string {
	return fmt.Sprintf(`
