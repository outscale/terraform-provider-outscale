...
func dataSourceOutscaleOAPINics() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			// This is attribute part for schema Nics
			// Argument
...
func resourceOutscaleOAPIDataSourceNicsDetach(oa []interface{}, meta interface{}, eniId string) error {
	...
	if oa != nil && len(oa) > 0 && oa[0] != nil {
		...
		stateConf := &resource.StateChangeConf{
			Pending: []string{"true"},
			Target:  []string{"false"},
			Refresh: networkInterfacesDataSourceOAPIAttachmentRefreshFunc(conn, eniId),
...
func networkInterfacesDataSourceOAPIAttachmentRefreshFunc(conn *fcu.Client, id string) resource.StateRefreshFunc {
