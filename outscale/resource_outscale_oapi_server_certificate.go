...
func resourceOutscaleOAPServerCertificate() *schema.Resource {
	return &schema.Resource{
		Create: resourceOutscaleOAPServerCertificateCreate,
		Read:   resourceOutscaleOAPServerCertificateRead,
		Delete: resourceOutscaleOAPServerCertificateDelete,
		Update: resourceOutscaleOAPServerCertificateUpdate,
		Importer: &schema.ResourceImporter{
			State: resourceOutscaleOAPServerCertificateImport,
		},
		Schema: map[string]*schema.Schema{
			...
			"server_certificate_name": {
				...
				Optional:     true,
				Computed:     true,
				ValidateFunc: oAPIValidateMaxLength(128),
			},
...
func resourceOutscaleOAPServerCertificateCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceOutscaleOAPServerCertificateRead(d, meta)
}
func resourceOutscaleOAPServerCertificateRead(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*OutscaleClient).EIM
...
func resourceOutscaleOAPServerCertificateUpdate(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*OutscaleClient).EIM
...
func resourceOutscaleOAPServerCertificateDelete(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*OutscaleClient).EIM
		...
		if err != nil {
			if awsErr, ok := err.(awserr.Error); ok {
				if awsErr.Code() == "DeleteConflict" && strings.Contains(awsErr.Message(), "currently in use by arn") {
					oAPICurrentlyInUseBy(awsErr.Message(), connLBU)
...
func resourceOutscaleOAPServerCertificateImport(
	d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	d.Set("server_certificate_name", d.Id())
	// private_key can't be fetched from any API call
	return []*schema.ResourceData{d}, nil
}
func oAPICurrentlyInUseBy(awsErr string, conn *lbu.Client) {
...
func oAPINormalizeCert(cert interface{}) string {
	...
	cleanVal := sha1.Sum(oAPIStripCR([]byte(strings.TrimSpace(rawCert))))
...
func oAPIValidateMaxLength(length int) schema.SchemaValidateFunc {
