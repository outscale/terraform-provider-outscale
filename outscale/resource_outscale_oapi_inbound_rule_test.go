...
func testAccCheckOutscaleOAPIRuleExists(n string, group *fcu.SecurityGroup) resource.TestCheckFunc {
...
func testAccCheckOutscaleOAPIRuleAttributes(n string, group *fcu.SecurityGroup, p *fcu.IpPermission, ruleType string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]
		if !ok {
