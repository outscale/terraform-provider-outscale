...
func TestAccOutscalePolicyGroup_basic(t *testing.T) {
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckOutscalePolicyGroupNameChanged(&groupPolicy1, &groupPolicy2),
				),
			},
		},
	})
...
func testAccCheckEIMGroupPolicyDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		group, name := resourceOutscalePolicyGroupParseID(rs.Primary.ID)
...
func testAccCheckEIMGroupPolicyExists(
	...
	return func(s *terraform.State) error {
		...
		group, name := resourceOutscalePolicyGroupParseID(policy.Primary.ID)
...
func testAccCheckOutscalePolicyGroupNameChanged(i, j *eim.GetGroupPolicyOutput) resource.TestCheckFunc {
	return func(s *terraform.State) error {
...
func testAccCheckOutscalePolicyGroupNameMatches(i, j *eim.GetGroupPolicyOutput) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if aws.StringValue(i.GetGroupPolicyResult.PolicyName) != aws.StringValue(j.GetGroupPolicyResult.PolicyName) {
