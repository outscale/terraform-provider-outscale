...
func resourceOutscaleLoadBalancerAttributesCreate(d *schema.ResourceData, meta interface{}) error {
	...
	acc["emit_interval"] = strconv.Itoa(int(aws.Int64Value(access.EmitInterval)))
	acc["enabled"] = strconv.FormatBool(aws.BoolValue(access.Enabled))
	acc["s3_bucket_name"] = aws.StringValue(access.S3BucketName)
	acc["s3_bucket_prefix"] = aws.StringValue(access.S3BucketPrefix)
	ac["access_log"] = acc

	l := make([]map[string]interface{}, 1)
	l[0] = ac

	if err := d.Set("load_balancer_attributes", l); err != nil {
		return err
	}
...
func resourceOutscaleLoadBalancerAttributesUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("load_balancer_attributes") {
		o, n := d.GetChange("load_balancer_attributes")
		_, n1 := d.GetChange("load_balancer_name")

		fmt.Printf("OLD => %+v", o.([]interface{})[0].(map[string]interface{}))
		fmt.Printf("NEW => %+v", n.([]interface{})[0].(map[string]interface{}))

		elbOpts := &lbu.ModifyLoadBalancerAttributesInput{
			LoadBalancerName: aws.String(n1.(string)),
		}

		lb := n.([]interface{})[0].(map[string]interface{})
