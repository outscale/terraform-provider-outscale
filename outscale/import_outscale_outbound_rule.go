...
func resourceOutscaleOutboundImportState(
	...
	for ruleType, perms := range permMap {
		for _, perm := range perms {
			ds, err := resourceOutscaleOutboundImportStatePerm(sg, ruleType, perm)
			if err != nil {
				return nil, err
			}
...
func resourceOutscaleOutboundImportStatePerm(sg *fcu.SecurityGroup, ruleType string, perm *fcu.IpPermission) ([]*schema.ResourceData, error) {
	/*
	...
	*/
	...
	if perm.IpRanges != nil {
		...
		r, err := resourceOutscaleOutboundImportStatePermPair(sg, ruleType, p)
		if err != nil {
			return nil, err
		}
...
func resourceOutscaleOutboundImportStatePermPair(sg *fcu.SecurityGroup, ruleType string, perm *fcu.IpPermission) (*schema.ResourceData, error) {
	...
	ruleResource := resourceOutscaleOutboundRule()
	d := ruleResource.Data(nil)
	d.SetId(id)
	d.SetType("outscale_outbound_rule")
	d.Set("group_id", sgId)
