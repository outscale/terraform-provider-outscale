...
func TestAccOutscalePolicyUserLink_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccCheckOutscalePolicyUserLinkDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccOutscaleUserPolicyAttachConfig(rName, policyName1),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckOutscalePolicyUserLinkExists("outscale_policy_user_link.test-attach", 1, &out),
					testAccCheckOutscalePolicyUserLinkAttributes([]string{policyName1}, &out),
				),
			},
			{
				Config: testAccOutscaleUserPolicyAttachConfigUpdate(rName, policyName1, policyName2, policyName3),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckOutscalePolicyUserLinkExists("outscale_policy_user_link.test-attach", 2, &out),
					testAccCheckOutscalePolicyUserLinkAttributes([]string{policyName2, policyName3}, &out),
				),
			},
		},
	})
...
func testAccCheckOutscalePolicyUserLinkExists(n string, c int, out *eim.ListAttachedUserPoliciesOutput) resource.TestCheckFunc {
...
func testAccCheckOutscalePolicyUserLinkAttributes(policies []string, out *eim.ListAttachedUserPoliciesOutput) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		matched := 0
