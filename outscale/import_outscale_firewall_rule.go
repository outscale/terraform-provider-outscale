...
func resourceOutscaleSecurityGroupImportState(
	...
	for ruleType, perms := range permMap {
		for _, perm := range perms {
			ds, err := resourceOutscaleSecurityGroupImportStatePerm(sg, ruleType, perm)
			if err != nil {
				return nil, err
			}
...
func resourceOutscaleSecurityGroupImportStatePerm(sg *fcu.SecurityGroup, ruleType string, perm *fcu.IpPermission) ([]*schema.ResourceData, error) {
	/*
	...
	*/
	...
	if perm.IpRanges != nil {
		...
		r, err := resourceOutscaleSecurityGroupImportStatePermPair(sg, ruleType, p)
		if err != nil {
			return nil, err
		}
...
func resourceOutscaleSecurityGroupImportStatePermPair(sg *fcu.SecurityGroup, ruleType string, perm *fcu.IpPermission) (*schema.ResourceData, error) {
	...
	ruleResource := resourceOutscaleFirewallRulesSet()
	d := ruleResource.Data(nil)
	d.SetId(id)
	d.SetType("outscale_firewall_rules_set")
	d.Set("group_id", sgId)
