package outscale

import (
	"fmt"
	"os"
	"strconv"
	"testing"

	"github.com/hashicorp/terraform/helper/acctest"
	"github.com/hashicorp/terraform/helper/resource"
)

func TestAccOutscaleOAPIDSEIMServerCertificates_basic(t *testing.T) {
	o := os.Getenv("OUTSCALE_OAPI")

	oapi, err := strconv.ParseBool(o)
	if err != nil {
		oapi = false
	}

	if !oapi {
		t.Skip()
	}

	rInt := acctest.RandInt()
	unixFile := "test-fixtures/eim-ssl-unix-line-endings.pem"
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		Providers:    testAccProviders,
		CheckDestroy: testAccCheckEIMServerCertificateDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccAPIDSEIMServerCertsConfigFile(rInt, unixFile),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("outscale_server_certificate.test_cert", "server_certificate_name"),
					resource.TestCheckResourceAttrSet("outscale_server_certificate.test_cert", "id"),
					resource.TestCheckResourceAttr("data.outscale_server_certificates.test", "server_certificate_metadata_list.#", "1"),
				),
			},
		},
	})
}

func testAccAPIDSEIMServerCertsConfigFile(rInt int, fName string) string {
	return fmt.Sprintf(`
resource "outscale_server_certificate" "test_cert" {
  server_certificate_name = "terraform-test-cert-%d"
  path = "/test_path/"
	server_certificate_body = "${file("%s")}"
	private_key =  <<EOF
-----BEGIN RSA PRIVATE KEY-----

detWVr2WRvgNgQvcRnNPECwfq1RtMJJpavaI3kgeaSxg
-----END RSA PRIVATE KEY-----
EOF
}

data "outscale_server_certificates" "test"{
	path = "${outscale_server_certificate.test_cert.path}"
}
	`, rInt, fName)
}
