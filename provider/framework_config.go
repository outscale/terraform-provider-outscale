...
func newOKSClientFW(data *ProviderModel) (oksClient *sdkv3_oks.Client, err error) {
	...
	logger := sdkv3_utils.WithLogging(logging.NewTflogWrapper())
...
func newAPIClientFW(data *ProviderModel) (apiClient *oscgo.APIClient, err error) {
	...
	httpClient.Transport = transport.NewTransport(data.AccessKeyId.ValueString(), data.SecretKeyId.ValueString(), data.Region.ValueString(), httpClient.Transport)
