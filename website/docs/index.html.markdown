...
description: |-
  The Outscale Services (AWS) provider is used to interact with the many resources supported by Outscale. The provider needs to be configured with the proper credentials before it can be used.
---
...
# Create a web server
resource "outscale_instance" "web" {
  # ...
}
```
...
`SECRET_ACCESS_KEY`, environment variables, representing your AWS
