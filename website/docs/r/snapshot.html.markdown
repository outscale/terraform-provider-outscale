...
* `snapshot_size` - (Optional) The size of the snapshot created in your account, in bytes. This size must be exactly the same as the source snapshot one. The maximum allowed size is 14901 bytes.
