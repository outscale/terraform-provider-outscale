...
* `bsu_optimized` - (Optional) If `true`, the VM is created with optimized BSU I/O. Specifying this parameter will restart the VM.
...
* `performance` - (Optional) The performance of the VM (`medium` \| `high` \|  `highest`). Specifying this parameter will restart the VM.
...
* `vm_type` - (Optional) The type of VM (`t2.small` by default). Specifying this parameter will restart the VM.<br />
