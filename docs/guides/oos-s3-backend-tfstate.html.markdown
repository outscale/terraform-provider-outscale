---
page_title: "OUTSCALE: Terraform Remote State on OOS (S3 Backend)"
description: "How to use OUTSCALE Object Storage (OOS) as an S3 backend for Terraform state (tfstate)."
---

# Terraform Remote State on OUTSCALE OOS (S3 Backend)

This guide explains how to store Terraform state (`tfstate`) in **OUTSCALE Object Storage (OOS)** using Terraform's built-in **S3 backend** (S3-compatible).

## Prerequisites

- Terraform installed (1.x recommended)
- An existing OOS bucket dedicated to Terraform state
...
- `<OOS_ENDPOINT>`: `https://oos.<OOS_REGION>.outscale.com`  
  Example: `https://oos.eu-west-2.outscale.com`
- `<TFSTATE_BUCKET>`: e.g. `myteam-tfstate`
...
See OUTSCALE documentation: https://docs.outscale.com/fr/userguide/s3cmd.html
...
[https://docs.outscale.com/fr/userguide/Avertissement-sur-la-compatibilit%C3%A9-des-SDK-et-de-la-CLI-AWS.html](https://docs.outscale.com/fr/userguide/Avertissement-sur-la-compatibilit%C3%A9-des-SDK-et-de-la-CLI-AWS.html)
...
terraform {
  ...
  backend "s3" {
    ...
    profile = "default"
    shared_credentials_files = ["./osc/config.json"]
    region  = "eu-west-2"
...
You should see the object corresponding to `<TFSTATE_KEY>`.
...
Reference:
[Avertissement sur la compatibilité des SDK et de la CLI AWS](https://docs.outscale.com/fr/userguide/Avertissement-sur-la-compatibilit%C3%A9-des-SDK-et-de-la-CLI-AWS.html)

### Error: "No valid credential sources found"

* Ensure `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` are set, or
* Ensure `~/.osc/config.json` has the profile you configured in the backend block, and `profile = "..."` matches.
