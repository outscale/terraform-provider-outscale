...
func (d *dataSourceQuota) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	dataState.QuotaType = to.String(ptr.From(quotaType.QuotaType))
	dataState.Name = to.String(ptr.From(quota.Name))
	dataState.Description = to.String(ptr.From(quota.Description))
	dataState.MaxValue = to.Int64(int64(ptr.From(quota.MaxValue)))
	dataState.UsedValue = to.Int64(int64(ptr.From(quota.UsedValue)))
	dataState.QuotaCollection = to.String(ptr.From(quota.QuotaCollection))
	dataState.ShortDescription = to.String(ptr.From(quota.ShortDescription))
	dataState.AccountId = to.String(ptr.From(quota.AccountId))
	dataState.Filter = flatenFilters
	dataState.Id = to.String(id.UniqueId())
	dataState.RequestId = to.String(ptr.From(respApi.ResponseContext.RequestId))
