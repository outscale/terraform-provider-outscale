...
func TestAccVM_WithImageLaunchPermission_Basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Scaffold everything
			{
				Config: testAccOutscaleImageLaunchPermissionConfig(omi, oapi.TestAccVmType, region, accountID, keypair, true, rInt, sgName),
			...
			{
				Config: testAccOutscaleImageLaunchPermissionConfig(omi, oapi.TestAccVmType, region, accountID, keypair, false, rInt, sgName),
				Check: resource.ComposeTestCheckFunc(
...
func TestAccVM_ImageLaunchPermissionDestruction_Basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Scaffold everything
			{
				Config: testAccOutscaleImageLaunchPermissionCreateConfig(omi, oapi.TestAccVmType, region, keypair, rInt, true, false, sgName),
				Check: resource.ComposeTestCheckFunc(
					testCheckResourceOAPILPIGetAttr("outscale_image.outscale_image", "id", &imageID),
			...
			{
				Config: testAccOutscaleImageLaunchPermissionCreateConfig(omi, oapi.TestAccVmType, region, keypair, rInt, true, true, sgName),
				Check: resource.ComposeTestCheckFunc(
					testCheckResourceOAPILPIGetAttr("outscale_image.outscale_image", "id", &imageID),
				),
			},
...
func testAccOutscaleImageLaunchPermissionExists(accountID string, imageID *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if has, err := oapi.HasOAPILaunchPermission(client, *imageID); err != nil {
			return err
		} else if !has {
