...
func GetOAPIVMNetworkInterfaceLightSet(respNics []osc.NicLight) ([]map[string]interface{}, []map[string]interface{}) {
	...
	for _, nic := range respNics {
		...
		nicMap := map[string]interface{}{
			"delete_on_vm_deletion":  nic.LinkNic.DeleteOnVmDeletion, // Workaround.
			"device_number":          nic.LinkNic.DeviceNumber,
...
func ParseVPNclientectionRouteID(ID string) (destinationIPRange, vpnclientectionID string) {
