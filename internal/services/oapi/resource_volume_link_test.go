...
func TestAccVM_WithVolumeAttachment_Basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccOAPIVolumeAttachmentConfig(omi, oapi.TestAccVmType, utils.GetRegion(), keypair, sgName),
...
func TestAccVM_WithVolumeAttachment_Migration(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: testacc.FrameworkMigrationTestSteps("1.1.3",
			testAccOAPIVolumeAttachmentConfig(omi, oapi.TestAccVmType, utils.GetRegion(), keypair, sgName),
		),
	})
}
