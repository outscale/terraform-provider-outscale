...
func ResourceOutscaleLinkRouteTableRead(d *schema.ResourceData, meta interface{}) error {
	routeTableID := d.Get("route_table_id").(string)
	linkRTable, err := readOutscaleLinkRouteTable(meta.(*client.OutscaleClient), routeTableID, d.Id())
	if err != nil {
		return err
	}
	if linkRTable == nil {
...
func readOutscaleLinkRouteTable(meta *client.OutscaleClient, routeTableID, linkRouteTableID string) (*oscgo.LinkRouteTable, error) {
	conn := meta.OSCAPI
