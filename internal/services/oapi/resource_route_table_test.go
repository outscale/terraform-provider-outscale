...
func TestAccNet_RouteTable_Instance(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccOAPIRouteTableConfigInstance(omi, oapi.TestAccVmType, utils.GetRegion()),
				Check: resource.ComposeTestCheckFunc(
...
func TestAccNet_WithRouteTable_Migration(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: testacc.FrameworkMigrationTestSteps("1.1.3",
			...
			testAccOAPIRouteTableConfigInstance(omi, oapi.TestAccVmType, utils.GetRegion())),
	})
}
